package com.etm.net
{
	 ...
	 */
	[Event(type="com.etm.event.SyncEvent", name="sync_start")]
	/**
	 *同步完成时进行派发
	 * @eventType com.xingcloud.net.SyncEvent
	 */
	[Event(type="com.etm.event.SyncEvent", name="sync_complete")]
	/**
	 *同步重试时派发
	 * @eventType com.xingcloud.net.SyncEvent
	 */
	[Event(type="com.etm.event.SyncEvent", name="sync_retry")]
	/**
	 *同步失败时派发
	 * @eventType com.xingcloud.net.SyncEvent
	 */
	[Event(type="com.etm.event.SyncEvent", name="sync_error")]
	/**
	 *
	...
	public class RequestPacker extends EventDispatcher implements ITick
	{
		...
		protected function onComplete(event:TaskEvent):void
		{
			...
			_blocked=false;
			var result:Object=(event.task as Connector).data;
			...
			{
				Debug.error("Message {0} has {1} request but get {2} response.", this, result.id, cache.length, batch.length);
				validate=false;
			}
			if (validate)
			{
				...
				for (var i:int=0; i < batch.length; i++)
				{
					...
					if (obj.index in cache)
						...
						Debug.error("Can't find request handler for messsage id:" + result.id + " index:" + obj.index, this);
					if (pr)
...
//			}
//			var connector:Connector=new com.etm.core.Etm(_mainService, batch, Etm.needAuth, retryTime);

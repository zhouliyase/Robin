package com.etm.starling.display
{
	...
	public class LargeImage extends Image
	{
		...
		{
			return _drawArea.width > 0 && _drawArea.height > 0;
		}
		...
		private function computeViewport():Rectangle
		{
			if (_viewportDirty)
			{
				...
				RectTransform.scaleRect(viewport, 1 / scaleX, 1 / scaleY);
				viewport.x-=this.x;
				viewport.y-=this.y;
		...
		public override function getBounds(targetSpace:DisplayObject, resultRect:Rectangle=null):Rectangle
		{
			...
			else
			{
				...
				var corners:Vector.<Point>=
					new <Point>[_boundingRect.topLeft, new Point(_boundingRect.right, _boundingRect.top), new Point(_boundingRect.left, _boundingRect.bottom),
					 _boundingRect.bottomRight];
				for (i=0; i < 4; ++i)
				{

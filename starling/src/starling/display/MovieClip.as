...
package starling.display
{
	...
	public class MovieClip extends Image implements IAnimatable
	{
		...
		private var mcurrentLabels:Vector.<String>;
		...
		public function MovieClip(textures:Vector.<Texture>, fps:Number=12)
		{
			if (textures.length > 0)
			{
				...
				mcurrentLabels=new Vector.<String>;
		...
		public function play(target:Object=null):void
		{
			mPlaying=true;
			if (target is int)
			{
		...
		{
			if (mLoop && mCurrentTime == loopEndTime)
				mCurrentTime=loopStartTime;
			if (!mPlaying || passedTime == 0.0 || mCurrentTime == loopEndTime)
				return;
			...
			while (mCurrentTime >= mStartTimes[mCurrentFrame] + mDurations[mCurrentFrame])
			{
				if (mCurrentFrame++ == loopEnd)
				{
					if (hasEventListener(Event.COMPLETE))
					{
						...
						if (!mPlaying)
						{
							mCurrentTime=loopEndTime;
							mCurrentFrame=loopEnd;
							break;
						}
					}
		...
		public function get currentLabels():Vector.<String>
		{
			return mcurrentLabels;
		}

		public function set currentLabels(value:Vector.<String>):void
		{
			mcurrentLabels=value;
		}

		public function get loopStartTime():Number
		{
			return mloopStart == -1 ? 0.0 : mStartTimes[mloopStart];
		}

		public function get loopEndTime():Number
		{
			return mloopEnd == -1 ? totalTime : (mStartTimes[mloopEnd] + mDurations[mloopEnd]);
		}

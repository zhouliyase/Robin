package com.etm.starling.display
{
	...
	import com.etm.starling.events.CameraUpdateEvent;
	import com.etm.geom.InfiniteRectangle;
	import com.etm.utils.AngleUtil;
	...
	[Event(name="cameraUpdate", type="com.etm.starling.events.CameraUpdateEvent")]
	/**
	...
	public class CameraSprite extends Sprite
	{
		 ...
		 */
		public static const NO_BOUNDS:InfiniteRectangle=
			new InfiniteRectangle(Number.NEGATIVE_INFINITY, Number.NEGATIVE_INFINITY, Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY);
		...
		{
			value ? _startPosition=value : new Point();
		}
		 ...
		 */
		public function get easingPan():Number
		{
			return _easingPan;
		}

		public function set easingPan(value:Number):void
		{
			_easingPan=DigitalUtil.clamp(value, 0, 1);
		}
		 ...
		 */
		public function get easingZoom():Number
		{
			return _easingZoom;
		}

		public function set easingZoom(value:Number):void
		{
			_easingZoom=DigitalUtil.clamp(value, 0, 1);
		}
		 ...
		 */
		public function get easingRotate():Number
		{
			return _easingRotate;
		}

		public function set easingRotate(value:Number):void
		{
			_easingRotate=DigitalUtil.clamp(value, 0, 1);
		}
		...
		public function moveTo(xPos:Number, yPos:Number, zoom:Number=1, rot:Number=0, force:Boolean=false):void
		{
			...
			{
				_targetX=-DigitalUtil.clamp(xPos, _boundingRect.left, _boundingRect.right);
				_targetY=-DigitalUtil.clamp(yPos, _boundingRect.top, _boundingRect.bottom);
			}
			if (force || _easingZoom != NO_MOTION)
			{
				_targetZoom=DigitalUtil.clamp(zoom, 0, Number.POSITIVE_INFINITY);
			}
		...
		private function stepAnimation(e:EnterFrameEvent):void
		{
			...
			if (!_shaking && DigitalUtil.nearEquals(_world.x, _targetX) && DigitalUtil.nearEquals(_world.y, _targetY) && DigitalUtil.nearEquals(_harness.scaleX, _targetZoom) &&
				DigitalUtil.nearEquals(_harness.getRotation(), _targetRot))
			{
				stopMoving();
			}
			dispatchEvent(new CameraUpdateEvent(this.viewport));
...
//FIXME: Rectangle roation
		 ...
		 */
		public function shake(intensity:Number=5, pulses:Number=5, interval:Number=.016666666):void
		{
			if (pulses < 1)
			{
				return;
			}
			_shaker=new DelayedCall(doShake, interval, [intensity]);
...
import com.etm.starling.display.CameraSprite;

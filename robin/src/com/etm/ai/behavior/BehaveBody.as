...
{
	import com.etm.geom.Circle;
	import com.etm.geom.Vector2D;

	public class BehaveBody extends MoveBody
	{
		private var _maxForce:Number=0.5;
		...
		public function pursue(target:MoveBody):void
		{
		...
		public function evade(target:MoveBody):void
		{
			var lookAheadTime:Number=position.dist(target.position) / _maxSpeed;
		...
		public function avoid(circles:Array):int
		{
			var avoidCount:int=0;
			for (var i:int=0; i < circles.length; i++)
			{
				...
				var cp:Vector2D=new Vector2D(circle.center.x, circle.center.y);
				var difference:Vector2D=cp.subtract(_position);
		...
		public function flock(vehicles:Array):void
		{
			...
			{
				var vehicle:MoveBody=vehicles[i] as MoveBody;
		...
		public function inSight(vehicle:MoveBody):Boolean
		{
		...
		public function tooClose(vehicle:MoveBody):Boolean
		{
